...
public class FacetsTest extends BaseTest {
	...
	public static void setUpBeforeClass() throws Exception {
		String attributeUrl= getProperty(BT_REST_URL) + "attributes";
		...
		for (int j = 0; j <facetAttributeRows.length(); j++) {
			...
			getAttributeID().put(attributeName, attributeId);
		}
	}
	...
	public void testCreate(Facet facet) throws Exception{
		...
		for (String attributeNameValue : getAttributeID().keySet()) {
			Integer attributeId = getAttributeID().get(attributeNameValue);
			String attributeIdValue = attributeId.toString();
			if(attributeName.equalsIgnoreCase(attributeNameValue))
			{
				facet.setAttributeId(attributeIdValue);
			}
		}
